    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if(IsSourceControlTimeOut){
                ...
                WriteI -message "appserviceCodeSyncSuccess $($appserviceCodeSyncSuccess)"
...
function ADAppUpdate {
    ...
    WriteI -message "`nUpdating the app..."
...
function CreateAdAppPrincipal {
    ...
    az logout
    WriteW -message "`nPlease inform your admin to consent the app permissions from this link`nhttps://login.microsoftonline.com/$tenantId/adminconsent?client_id=$appId"
}    
...
# Start Deployment.
Write-Ascii -InputObject "AskAway v2" -ForegroundColor Magenta
