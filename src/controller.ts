...
export const getUpdatedMainCard = async (
    ...
    try {
        ...
        const questionData = <IQuestionPopulatedUser[]>(
            (await db.getQuestionData(qnaSessionId)).map((v) => v.toObject())
        );

        const topQuestions = questionData.sort((a, b) => {
            // Sort by votes then most recent
        ...
        const newestQuestions = questionData.sort((a, b) => {
            return b.dateTimeCreated.getTime() - a.dateTimeCreated.getTime();
        });
        ...
        return ok({
            card: await getMainCard(
                ...
                topQuestions.slice(0, 3), // Take three oldest questions
                newestQuestions,
                numQuestions
            ),
